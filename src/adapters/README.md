...
- `core/target.js` (`setTargetRuntime(ctx, hooks)`): `load`, `syncDensity`, `isCanvasReady`, `instance`, `activateInstance`, `deactivateInstance`, `getActiveFramebuffer`, `isFramebufferTarget`, installed on a context as `ctx.targetHooks`. The target itself is per-context too (`ctx.renderer`, `ctx.width`, `ctx.height`, `ctx.density`), written with `setTarget(ctx, …)`; the standalone adapter keeps the canvas it was loaded from and that target's ready promise in `ctx.target`.
